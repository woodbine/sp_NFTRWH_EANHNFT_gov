...
entity_id = "NFTRWH_EANHNFT_gov"
url = "http://www.enhertsccg.nhs.uk/spend-over-25000"
errors = 0
data = []
...
blocks = soup.find('div', 'view-content').find_all('td')
for block in blocks:
    try:
        if '.csv' in block.find('a')['href'] or '.xls' in block.find('a')['href'] or '.xlsx' in block.find('a')['href'] or '.pdf' in block.find('a')['href']:
            ...
            if 'Month 02' in title:
                csvMth = 'May'
            if 'Month 03' in title:
                csvMth = 'June'
            if 'Month 04' in title:
                csvMth = 'July'
            if 'Month 05' in title:
                csvMth = 'August'
            if 'Month 06' in title:
                csvMth = 'September'
            if 'Month 07' in title:
                csvMth = 'October'
            if 'Month 08' in title:
                csvMth = 'November'
            if 'Month 09' in title:
                csvMth = 'December'
